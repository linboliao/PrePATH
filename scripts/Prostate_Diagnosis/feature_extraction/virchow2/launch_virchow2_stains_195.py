#!/usr/bin/env python
"""Re-extract virchow2 features for the 301 (+ ynzl) external cohorts WITH stain
normalisation, on the Code-195 box.
...
virchow2, then the frozen 5-fold AB_MIL ensemble is re-evaluated.
  ...
  - output   : <FEAT_ROOT>/MIL外部测试/feat_0_224_stains/<Method>/{pt_files,h5_files}/virchow2/
...
def main():
    ...
    shard_root = os.path.join(PRE, 'csv', f'ProstateDiagnosis_virchow2_stains_{a.method}')
    log_dir = os.path.join(PRE, 'logs', f'virchow2_stains_{a.method}')
    os.makedirs(shard_root, exist_ok=True)
    os.makedirs(log_dir, exist_ok=True)
    os.makedirs(feat_dir, exist_ok=True)

    widx = wsi_index()
    done_dir = os.path.join(feat_dir, 'pt_files', MODEL)
    ...
    print(f'method={a.method}  ref={a.ref}')
