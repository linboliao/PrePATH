...
def mp_seg_and_patch(
    ...
    def func(i):
        ...
        patch_time_elapsed = -1  # Default time
        if patch:
            if WSI_object.object_power:
                scale = WSI_object.object_power / 20
                adjusted_patch_size = int(patch_size * scale)
                adjusted_step_size = int(step_size * scale)
            current_patch_params.update({"patch_level": patch_level, "patch_size": adjusted_patch_size, "step_size": adjusted_step_size, "save_path": patch_save_dir})
            file_path, patch_time_elapsed = patching(
                WSI_object=WSI_object,
                **current_patch_params,
            )
...
if __name__ == "__main__":
    ...
    if args.use_mp:
        ...
        fn = seg_and_patch
    seg_times, patch_times = fn(**directories, **parameters, patch_size=args.patch_size, step_size=args.step_size, seg=args.seg, use_default_params=False, save_mask=True, stitch=args.stitch, patch_level=args.patch_level, patch=args.patch,
                                process_list=process_list, auto_skip=args.no_auto_skip, wsi_format=wsi_format)
